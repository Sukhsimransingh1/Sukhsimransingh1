
<h1 align="center">Hi 👋, I'm Sukhsimran Singh</h1>

<h3 align="center">
🚀 AI/ML Engineer | Data Scientist | Generative AI Developer
</h3>

<p align="center">
  <a href="https://www.linkedin.com/in/sukhsimran-singh1/">LinkedIn</a> •
  <a href="https://github.com/Sukhsimransingh1">GitHub</a> •
  <a href="mailto:sukhsimransingh304@gmail.com">Email</a> •
  <a href="https://sukhsimran-singh.lovable.app/">Portfolio</a> •
  <a href="https://drive.google.com/file/d/1ZR5YIIIl94-IFtNwgJQ_FRTRcINTcSti/view?usp=sharing">Resume</a>
</p>

---

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=3000&color=00F7FF&center=true&vCenter=true&width=750&lines=AI%2FML+Engineer;Data+Scientist;Generative+AI+Developer;Building+Intelligent+Applications;Machine+Learning+%7C+LLMs+%7C+RAG+%7C+Python" />
</p>

---

# 💡 About Me

- 🎓 B.Tech Computer Science & Engineering Student (3rd Year)
- 🤖 Passionate about Artificial Intelligence, Machine Learning, Data Science & Generative AI
- 🚀 Building intelligent applications using Machine Learning, LLMs and Retrieval-Augmented Generation (RAG)
- 🛠️ Experienced in developing end-to-end AI solutions from data preprocessing to deployment
- 📚 Continuously learning Agentic AI, Prompt Engineering, NLP and scalable AI systems
- 🎯 Aspiring AI Engineer focused on solving real-world problems through practical applications

---

# 🛠 Tech Stack

### 💻 Programming Languages

Python • SQL • C • C++

### 📊 Data Science & Analytics

Data Analysis • Exploratory Data Analysis (EDA) • Data Cleaning • Feature Engineering • Statistical Analysis • Data Visualization

### 🤖 Machine Learning & AI

Machine Learning • Deep Learning • Natural Language Processing • Computer Vision • Model Evaluation • Prompt Engineering

### 🧠 Generative AI & LLM Engineering

Large Language Models (LLMs) • LangChain • Retrieval-Augmented Generation (RAG) • Semantic Search • Vector Embeddings • Hugging Face • Groq API

### ⚙️ Frameworks & Libraries

FastAPI • Streamlit • Flask • Scikit-learn • TensorFlow • Pandas • NumPy • Matplotlib

### 🗄️ Databases

PostgreSQL • MySQL • ChromaDB

### 🚀 Developer Tools

Git • GitHub • Docker • VS Code • Jupyter Notebook • Google Colab

<p align="center">

<img src="https://skillicons.dev/icons?i=python,c,cpp,sql,mysql,postgres,tensorflow,docker,fastapi,flask,streamlit,git,vscode,pandas,numpy,matplotlib,sklearn" />

</p>

---

# 🚀 Current Focus

<p align="center">

Generative AI • LLM Engineering • Retrieval-Augmented Generation (RAG) • Machine Learning • Deep Learning • NLP • AI Agents • Full-Stack AI Applications

</p>

---

# 🔥 Featured Projects

## 🩺 PranRakshak AI — AI-Powered Sepsis Early Detection System

An intelligent healthcare platform designed to assist clinicians by predicting sepsis risk using machine learning and explainable AI techniques.

### Key Highlights

- Built an end-to-end sepsis prediction pipeline
- Implemented Sepsis-3 clinical criteria
- Integrated Explainable AI for prediction transparency
- Developed an interactive Streamlit interface
- Focused on real-world clinical decision support

**Tech Stack**

Python • Scikit-learn • Streamlit • FastAPI • Pandas

---
## 📚 OmniRAG AI — Multi-Document Research Assistant

A production-grade Retrieval-Augmented Generation (RAG) system that enables intelligent conversations across multiple uploaded documents.

### Key Highlights
📄 Built contextual AI chat over multiple PDFs with isolated retrieval pipelines to prevent cross-document leakage.
🔍 Implemented hybrid semantic retrieval using ChromaDB and LangChain orchestration.
💬 Integrated Llama 3 with streaming responses and multi-turn conversation memory.
⚡ Developed a full-stack architecture with FastAPI backend and Next.js frontend.

**Tech Stack**

LangChain • ChromaDB • Llama 3 • FastAPI • Next.js • RAG




---

# 📊 Contribution Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Sukhsimransingh1&theme=tokyo-night&hide_border=true" />
</p>

---

# 🏅 Highlights

<p align="center">
  <img src="https://img.shields.io/badge/Artificial%20Intelligence-blue?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Machine%20Learning-green?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Generative%20AI-purple?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/LLMs-orange?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/RAG-red?style=for-the-badge"/>
</p>

---

# 🏆 Achievements


- 🥇 Top Performer — Data Science Training (Internshala × IITM Pravartak × NSDC)
- 🏆 Top 15 — Innovate with TRAE, NIT Jalandhar
- 💻 Participant — Hack on Hills, NIT Hamirpur
- 🚀 Participant — Hackathon 4.0, GNA University
- 🤖 Built multiple production-grade AI, ML and Generative AI applications
---

# 📜 Certifications

- 🎓 Data Science Training — Internshala
- 🎓 Machine Learning & Data Science Certification — Udemy
- 🎓 Generative AI Learning (Ongoing)

---

# 🌱 Currently Exploring

- Agentic AI Systems
- Multi-Agent Workflows
- Advanced RAG Architectures
- LLM Fine-Tuning
- AI Deployment & MLOps
- Scalable AI Applications

---

# 🤝 Connect With Me

💼 LinkedIn: https://www.linkedin.com/in/sukhsimran-singh1/

💻 GitHub: https://github.com/Sukhsimransingh1

🌐 Portfolio: https://sukhsimran-singh.lovable.app/

📧 Email: sukhsimransingh304@gmail.com

---

# 🎯 Career Interests

- Artificial Intelligence
- Machine Learning
- Data Science
- Generative AI
- LLM Engineering
- NLP Applications
- AI Product Development
- Full-Stack AI Systems

---

<p align="center">

⚡ <b>Passionate about building AI-powered products, solving real-world problems, and continuously learning emerging technologies.</b>

</p>

